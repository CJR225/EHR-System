...
function PatientDash() {
  ...
  useEffect(() => {
    const fetchHistoryInfo = async () => {
      ...
      if (selectedPatient && activeTab === 'History') {
        ...
        try {
          const historyResponse = await axios.get(`http://localhost:3001/patients/patient-history/${selectedPatient.id}`);
  ...
  useEffect(() => {
    const fetchMedications = async () => {
      if (selectedPatient && (activeTab === 'Med Rec' || activeTab === 'MAR')) { // Include 'MAR' tab in condition
        ...
        <div className="sidebar">
          {
            menuItem.map((item, index) => (
              <NavLink to={item.path || "#"} key={index} className="sidebarLink">
                <div onClick={item.onClick} className="sidebarIcon active">{item.icon}</div>
                ...
                {activeTab === 'History' && historyInfo && (
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <p style={{ marginTop: '5px' }}>{historyInfo.history_of_presenting_problem}</p>
                    </div>
                    ...
                    <div style={{
                      ...
                      <p style={{ marginTop: '5px' }}>{historyInfo.past_medical_history}</p>
                    </div>
                    ...
                    <div style={{
                      ...
                      <p style={{ marginTop: '5px' }}>{historyInfo.family_history}</p>
                    </div>
                  </div>
                )}
                <div className={styles.someClassName}>
                  ...
                  {activeTab === 'MAR' && (
                    <div className={styles.tabContent}>
                      <h2 className={styles.tabHeading}>Medication Administration Record (MAR)</h2>
                      <form onSubmit={handleAddMedication} className={styles.marForm}>
                        <div className={styles.formRow}>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="med_id">Med ID</label>
                            <input className={styles.formInput} type="text" name="med_id" id="med_id" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="dosage">Dosage</label>
                            <input className={styles.formInput} type="number" name="dosage" id="dosage" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="route">Route</label>
                            <input className={styles.formInput} type="text" name="route" id="route" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="frequency">Frequency</label>
                            <input className={styles.formInput} type="text" name="frequency" id="frequency" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="taken_last">Last Taken</label>
                            <input className={styles.formInput} type="datetime-local" name="taken_last" id="taken_last" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="time_taken">Time Taken</label>
                            <input className={styles.formInput} type="datetime-local" name="time_taken" id="time_taken" />
                          </div>
                          <div className={styles.inputWrapper}>
                            <label htmlFor="administered_at">Administered At</label>
                            <input className={styles.formInput} type="datetime-local" name="administered_at" id="administered_at" required />
                          </div>
                        </div>
                        ...
                        <tbody>
                          {medications.map((med) => (
                            <tr key={med.id}>
                              ...
                              <td>
                                <button onClick={() => handleDeleteMedication(med.id)} className={`${styles.formButton} ${styles.delete}`}>Delete</button>
